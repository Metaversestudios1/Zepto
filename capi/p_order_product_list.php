  ...
  while($row = $sel->fetch_assoc())
    {
		...
		$sname = $mysqli->query("select * from vendor where id=".$row['sid']."")->fetch_assoc();
		$pp['p_method_name'] = $pname['title'];
		$pp['customer_address'] = $row['delivery_type'] == 0 ? $row['address'] : $sname['address'];
		$pp['delivery_type'] = $row['delivery_type'];
		$pp['timeslot'] = $row['timeslot'];
